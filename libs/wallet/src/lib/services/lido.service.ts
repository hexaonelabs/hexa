...
export class LidoService {
  public readonly balance$: BehaviorSubject<number> =
    new BehaviorSubject<number>(null as any);
  public readonly shares$: BehaviorSubject<number> =
    new BehaviorSubject<number>(null as any);
  ...
  async loadData(provider: ethers.providers.Web3Provider) {
    ...
    const stETHabi = await fetch('/assets/abi/stETH.abi.json').then((res) =>
      res.json()
    );
  ...
  async swapToLSToken(
    ...
    const { chainId } = await provider.getNetwork();
    const { STETH = undefined } = (TOKENS_BY_NETWORK as any)[chainId];
    const signer = provider.getSigner();
  ...
  async swapToWLSToken(
    ...
    const { chainId } = await provider.getNetwork();
    const { WSTETH = undefined } = (TOKENS_BY_NETWORK as any)[chainId];
    const signer = provider.getSigner();
    const sender = await signer.getAddress();
    // send transaction
    const tx = {
